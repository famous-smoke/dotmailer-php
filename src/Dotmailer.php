...
//            );
        ...
        public function getContactsFromAddressBook(int $addressbookID, $withFullData=false, $select = 1000, $skip = 0){
            $url = sprintf("/v2/address-books/%s/contacts?withFullData=%s&select=%s&skip=%s", $addressBookId, $withFullData, $select, $skip);
            $this->response = $this->adapter->get($url);
        ...
        public function getContactAddressBooks(Contact $contact): array
        {
            ...
            if($jsonStr !== '') {
                ...
                foreach ($res as $addressBook) {
                    $addressBooks[] = new AddressBook($addressBook->id, $addressBook->name, $addressBook->visibility,
                        $addressBook->contacts);
                }
            }
            return $addressBooks;
         ...
         * @param string|null $challengeUrl
         */
        public function resubscribeContactNoChallenge(Contact $contact, string $preferredLocale = null, string $challengeUrl = null)
        {
            $content = [
                'unsubscribedContact' => [
                    'email' => $contact->getEmail(),
